...
if 0:
	...
	df.tail()
if 1:
	f = open('rcode_to_scop_class_rstep0.01.csv','r')
...
possible_colors = [(31.0/255.0,120.0/255.0,180.0/255.0),
...
possible_markers = ['o', 'v', '^', '<', '>', '8', 's', 'p', '*', 'h', 'H', 'D', 'd']   
#('o', 'v', '^', '<', '>', '8', 's', 'p', '*', 'h', 'H', 'D', 'd')
color_count=0
for i in species_keys:
	#
	if color_count >= len(possible_colors):
		color_count = 0
	species_colors.append(possible_colors[color_count])
	color_count +=1
	#
	if marker_count >= len(possible_markers):
		marker_count = 0
