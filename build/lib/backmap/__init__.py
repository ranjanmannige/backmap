...
if __name__ == "__main__":
	...
	for chain in chains:
		...
		if showrcode:
			...
			draw_xyz(X,Y,Z,cmap=cmap, xlabel='Frame #',ylabel='Residue #',zlabel=r'$\mathcal{R}$', title="Chain: '%s'"%chain, vmin=0, vmax=1)
			write_image(fn)
		# ----------------------------------
		xy_to_z = {}
		...
		if len(set(X))>1:
			...
			filenames.append(fn)
			draw_xyz(X,Y,Z,cmap=plt.get_cmap("Reds"), xlabel='Frame #',ylabel='Residue #',zlabel=r'RMSD$_\mathcal{R}$', title="Chain: '%s'"%chain)
			write_image(fn)		
		# ----------------------------------
		...
		if len(set(X)) > 1:
			...
			filenames.append(fn)
			draw_xyz(X,Y,Z,cmap=plt.get_cmap("Blues"), xlabel='Frame #',ylabel='Residue #',zlabel=r'RMSF$_\mathcal{R}$', title="Chain: '%s'"%chain)
			write_image(fn)
		...
		if showhis:
			...
			filenames.append(fn)
			draw_xyz(X,Y,Z,cmap=plt.get_cmap("gray_r"), xlabel='Frame #',ylabel=r'$\mathcal{R}$',zlabel=r'$P(\mathcal{R})$', title="Chain: '%s'"%chain)
			write_image(fn)
		#
